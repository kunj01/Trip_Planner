...
const Review = ({ review, place }) => {
    ...
    const [showMoreResponse, setShowMoreResponse] = useState(false);
                ...
                <div className="w-full md:col-span-10 space-y-2 mt-2 md:mt-0">
                    ...
                    { review?.owner_response && (
                        <div className="pl-3 md:pl-4 border-l border-gray-300 my-2">
                            ...
                            {/* Show More Functionality to truncate long text for response */}
                            <p className="text-sm">
                                {/* If User has Clicked Show More or Response text is less than 46 words.... */}
                                { (showMoreResponse || !review?.owner_response?.text || review?.owner_response?.text?.split(' ').length <= 46) ? 
                                    // ...then all full response text is displayed 
                                    review?.owner_response?.text 
                                    // ...else, the response text is truncated to 45 words and the '...' character is appended to the text
                                    : `${review?.owner_response?.text?.split(' ').slice(0, 45).join(' ')}... ` 
                                } 
                                
                                {/* If Response text is greater than 45 words... */}
                                { (review?.owner_response?.text && review?.owner_response?.text?.split(' ').length > 45) && (
                                    // ...a More/Show Less state button is added to the truncated response text
                                    <span className="font-medium cursor-pointer" 
                                        // onClick event to toggle between show more / show less
                                        onClick={() => setShowMoreResponse(!showMoreResponse)} 
                                    >
                                        {/* Text displayed when response text is in Show more state (Displays 'Show less' when show more is clicked) or show less state (Displays 'More' when show less is clicked) */}
                                        { showMoreResponse ? <span><br />Show less</span> : 'More' }
                                        {/* --- */}                     
                                    </span>
                                ) }
                                {/* --- */}
